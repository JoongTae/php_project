...
<?php
	...
	$sql = "select * from musician_board where num=$num"; // 해당 글 번호의 게시글 조회 쿼리
	...
	$sql = "update musician_board set hit=$new_hit where num=$num"; // 글 조회수 업데이트 쿼리   
...
?>		
	    ...
	    <ul class="buttons">
                <li><button onclick="location.href='musician_board_list.php?page=<?=$page?>'">목록</button></li> <!-- 목록 버튼 클릭 시 리스트 페이지로 이동 -->
		<li><button onclick="location.href='musician_board_modify_form.php?num=<?=$num?>&page=<?=$page?>'">수정</button></li> <!-- 수정 버튼 클릭 시 수정 폼 페이지로 이동 -->
		<li><button onclick="location.href='musician_board_delete.php?num=<?=$num?>&page=<?=$page?>'">삭제</button></li> <!-- 삭제 버튼 클릭 시 삭제 처리 페이지로 이동 -->
		<li><button onclick="location.href='musician_board_form.php'">글쓰기</button></li> <!-- 글쓰기 버튼 클릭 시 글 작성 폼 페이지로 이동 -->
            </ul>
	</div> <!-- board_box -->
</section> 
<footer>
